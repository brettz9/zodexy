...
var codeInput = {
  ...
  registerTemplate: function (templateName, template) {
    if (!(typeof templateName == "string" || templateName instanceof String))
      throw TypeError(
        `code-input: Name of template "${templateName}" must be a string.`,
      );
    if (
      !(
        typeof template.highlight == "function" ||
        template.highlight instanceof Function
      )
    )
      throw TypeError(
        `code-input: Template for "${templateName}" invalid, because the highlight function provided is not a function; it is "${template.highlight}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
      );
    if (
      !(
        typeof template.includeCodeInputInHighlightFunc == "boolean" ||
        template.includeCodeInputInHighlightFunc instanceof Boolean
      )
    )
      throw TypeError(
        `code-input: Template for "${templateName}" invalid, because the includeCodeInputInHighlightFunc value provided is not a true or false; it is "${template.includeCodeInputInHighlightFunc}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
      );
    if (
      !(
        typeof template.preElementStyled == "boolean" ||
        template.preElementStyled instanceof Boolean
      )
    )
      throw TypeError(
        `code-input: Template for "${templateName}" invalid, because the preElementStyled value provided is not a true or false; it is "${template.preElementStyled}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
      );
    if (
      !(
        typeof template.isCode == "boolean" ||
        template.isCode instanceof Boolean
      )
    )
      throw TypeError(
        `code-input: Template for "${templateName}" invalid, because the isCode value provided is not a true or false; it is "${template.isCode}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
      );
    if (!Array.isArray(template.plugins))
      throw TypeError(
        `code-input: Template for "${templateName}" invalid, because the plugin array provided is not an array; it is "${template.plugins}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
      );

    template.plugins.forEach((plugin, i) => {
      if (!(plugin instanceof codeInput.Plugin)) {
        throw TypeError(
          `code-input: Template for "${templateName}" invalid, because the plugin provided at index ${i} is not valid; it is "${template.plugins[i]}". Please make sure you use one of the constructors in codeInput.templates, and that you provide the correct arguments.`,
        );
      }
    });
      ...
      return {
        highlight: function (resultElement, codeInput, plugins = []) {
          let characterLimit = Number(
            codeInput.getAttribute("data-character-limit"),
          );

          let normalCharacters = codeInput.escapeHtml(
            codeInput.value.slice(0, characterLimit),
          );
          let overflowCharacters = codeInput.escapeHtml(
            codeInput.value.slice(characterLimit),
          );
      ...
      return {
        highlight: function (resultElement, codeInput) {
          ...
          for (let i = 0; i < sections.length; i++) {
            htmlResult.push(
              `<span style="color: ${codeInput.template.rainbowColors[i % codeInput.template.rainbowColors.length]}">${codeInput.escapeHtml(sections[i])}</span>`,
            );
          }
          resultElement.innerHTML = htmlResult.join(
            codeInput.template.delimiter,
          );
        },
        includeCodeInputInHighlightFunc: true,
        preElementStyled: true,
      ...
      get(plugins, name) {
        if (plugins[name] == undefined) {
          throw ReferenceError(
            `code-input: Plugin '${name}' is not defined. Please ensure you import the necessary files from the plugins folder in the WebCoder49/code-input repository, in the <head> of your HTML, before the plugin is instatiated.`,
          );
        }
        return plugins[name];
      },
  ...
  CodeInput: class extends HTMLElement {
    ...
    update() {
      ...
      // Syntax Highlight
      if (this.templateObject.includeCodeInputInHighlightFunc)
        this.templateObject.highlight(resultElement, this);
    ...
    syncSize() {
      ...
      const height = getComputedStyle(
        this.getStyledHighlightingElement(),
      ).height;
    ...
    syncIfColorNotOverridden(callbackIfOverridden = function () {}) {
      ...
      window.requestAnimationFrame(() => {
        this.internalStyle.setProperty(
          "--code-input_no-override-color",
          "rgb(0, 0, 0)",
        );
        if (getComputedStyle(this).color == "rgb(0, 0, 0)") {
          ...
          if (getComputedStyle(this).color == "rgb(255, 255, 255)") {
            ...
            const highlightedTextColor = getComputedStyle(
              this.getStyledHighlightingElement(),
            ).color;

            this.internalStyle.setProperty(
              "--code-input_highlight-text-color",
              highlightedTextColor,
            );
            this.internalStyle.setProperty(
              "--code-input_default-caret-color",
              highlightedTextColor,
            );
    ...
    setKeyboardNavInstructions(instructions, includeAriaDescriptionFirst) {
      this.dialogContainerElement.querySelector(
        ".code-input_keyboard-navigation-instructions",
      ).innerText = instructions;
      if (includeAriaDescriptionFirst) {
        this.textareaElement.setAttribute(
          "aria-description",
          this.originalAriaDescription + ". " + instructions,
        );
      } else {
    ...
    setup() {
      ...
      const fallbackTextarea = this.querySelector(
      ...
      if (fallbackTextarea) {
        ...
        if (fallbackTextarea === document.activeElement) {
          // Thanks to https://stackoverflow.com/a/36430896
      ...
      this.originalAriaDescription =
        this.getAttribute("aria-description") || "Code input field";
      ...
      stylesheet.innerHTML =
        "code-input.code-input_styles_" + codeInput.stylesheetI + " {}";
      ...
      for (let i = 0; i < this.attributes.length; i++) {
        ...
        if (
          codeInput.textareaSyncAttributes.includes(attribute) ||
      ...
      keyboardNavigationInstructions.classList.add(
        "code-input_keyboard-navigation-instructions",
      );
      ...
      if (fallbackSelectionStart !== undefined) {
        textarea.setSelectionRange(
          ...
          fallbackSelectionEnd,
          fallbackSelectionDirection,
        );
      ...
      this.preElement.addEventListener(
        "-webkit-transitionend",
        preColorChangeCallback,
      );
      ...
      this.dialogContainerElement.addEventListener(
        "transitionend",
        thisColorChangeCallback,
      );
      this.dialogContainerElement.addEventListener(
        "-webkit-transitionend",
        thisColorChangeCallback,
        ...
        for (let i = 0; i < codeInput.observedAttributes.length; i++) {
          if (mutation.attributeName == codeInput.observedAttributes[i]) {
            this.attributeChangedCallback(
              ...
              mutation.oldValue,
              super.getAttribute(mutation.attributeName),
            );
          }
        }
      ...
      if (this.isConnected) {
        ...
        switch (name) {
          ...
          case "language":
            ...
            if (
              mainTextarea.placeholder == oldValue ||
              (oldValue == null && mainTextarea.placeholder == "")
            ) {
              if (newValue === null) {
          ...
          default:
            if (
              codeInput.textareaSyncAttributes.includes(name) ||
        ...
        if (fallbackTextarea) {
          ...
          if (defaultValue === undefined) {
            throw new Error(
              "Cannot get " +
                name +
                " of an unregistered code-input element without a data-code-input-fallback textarea.",
            );
          }
          return defaultValue;
        }
    ...
    set autocomplete(val) {
      return this.setAttribute("autocomplete", val);
    }
    get cols() {
      return this.getTextareaProperty(
        "cols",
    ...
    get form() {
      return this.getTextareaProperty("form");
    }
    get labels() {
      return this.getTextareaProperty("labels");
    }
    get maxLength() {
    ...
    get selectionDirection() {
      return this.getTextareaProperty("selectionDirection");
    }
    set selectionDirection(val) {
      this.setTextareaProperty("selectionDirection", val);
    }
    get selectionEnd() {
      return this.getTextareaProperty("selectionEnd");
    }
    set selectionEnd(val) {
      this.setTextareaProperty("selectionEnd", val);
    }
    get selectionStart() {
      return this.getTextareaProperty("selectionStart");
    }
    set selectionStart(val) {
      this.setTextareaProperty("selectionStart", val);
    }
    get textLength() {
    ...
    get validationMessage() {
      return this.getTextareaProperty("validationMessage");
    }
    get validity() {
      return this.getTextareaProperty("validationMessage");
    }
    get value() {
      return this.getTextareaProperty(
        "value",
        this.getAttribute("value") || this.innerHTML,
      );
    }
    set value(val) {
      val = val || "";
      ...
      if (this.textareaElement) {
        ...
        if (fallbackTextarea) {
          ...
          throw new Error(
            "Cannot call " +
              name +
              " on an unregistered code-input element without a data-code-input-fallback textarea.",
          );
        }
      }
    }
    ...
    blur(options = {}) {
      this.getTextareaMethod("blur")(options);
    }
    checkValidity() {
      return this.getTextareaMethod("checkValidity")();
    }
    focus(options = {}) {
      this.getTextareaMethod("focus")(options);
    }
    reportValidity() {
      return this.getTextareaMethod("reportValidity")();
    }
    setCustomValidity(error) {
      this.getTextareaMethod("setCustomValidity")(error);
    }
    setRangeText(
      replacement,
      ...
      this.getTextareaMethod("setRangeText")(
        replacement,
        selectionStart,
        selectionEnd,
        selectMode,
    ...
    setSelectionRange(
      ...
      this.getTextareaMethod("setSelectionRange")(
        selectionStart,
        selectionEnd,
        selectionDirection,
      );
...
// ESM-SUPPORT-START-TEMPLATES-BLOCK-2 Do not (re)move this - it's needed for ESM generation!
{
   ...
   */
  class Prism extends codeInput.Template {
    // Dependency: Prism.js (https://prismjs.com/)
    /**
   ...
   */
  class Hljs extends codeInput.Template {
    // Dependency: Highlight.js (https://highlightjs.org/)
    /**
