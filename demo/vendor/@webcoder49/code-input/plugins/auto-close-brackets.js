...
codeInput.plugins.AutoCloseBrackets = class extends codeInput.Plugin {
  ...
  afterElementsAdded(codeInput) {
    ...
    codeInput.textareaElement.addEventListener("keydown", (event) => {
      this.checkBackspace(codeInput, event);
    });
    codeInput.textareaElement.addEventListener("beforeinput", (event) => {
      this.checkClosingBracket(codeInput, event);
    });
    codeInput.textareaElement.addEventListener("input", (event) => {
      this.checkOpeningBracket(codeInput, event);
    });
  }
  ...
  checkClosingBracket(codeInput, event) {
    ...
    if (
      event.data ==
      codeInput.textareaElement.value[codeInput.textareaElement.selectionStart]
    ) {
      ...
      for (let openingBracket in this.bracketPairs) {
        ...
        if (event.data == closingBracket) {
          ...
          codeInput.textareaElement.selectionStart =
            codeInput.textareaElement.selectionEnd += 1;
  ...
  checkBackspace(codeInput, event) {
    ...
    if (
      ...
      let closingBracket =
        this.bracketPairs[
          codeInput.textareaElement.value[
