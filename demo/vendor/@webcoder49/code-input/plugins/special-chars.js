...
codeInput.plugins.SpecialChars = class extends codeInput.Plugin {
   ...
   */
  constructor(
    colorInSpecialChars = false,
    inheritTextColor = false,
    specialCharRegExp = /(?!\n)(?!\t)[\u{0000}-\u{001F}]|[\u{007F}-\u{009F}]|[\u{0200}-\u{FFFF}]/gu,
  ) {
    // By default, covers many non-renderable ASCII characters
  ...
  afterHighlight(codeInput) {
    ...
    codeInput.pluginData.specialChars.contrastColor =
      window.getComputedStyle(resultElement).color;
      ...
      if (nextNode.nodeType == 3) {
        ...
        if (searchResult != null) {
          ...
          if (nextNode.textContent != "") {
            let replacementElement = this.getStylisedSpecialChar(
              codeInput,
        ...
        if (
          nextNode.className != "code-input_special-char" &&
          nextNode.nodeValue != ""
        ) {
          // Element - recurse
  ...
  getStylisedSpecialChar(codeInput, matchChar) {
    ...
    result.style.setProperty(
      "--hex-0",
      "var(--code-input_special-chars_" + hexCode[0] + ")",
    );
    result.style.setProperty(
      "--hex-1",
      "var(--code-input_special-chars_" + hexCode[1] + ")",
    );
    result.style.setProperty(
      "--hex-2",
      "var(--code-input_special-chars_" + hexCode[2] + ")",
    );
    result.style.setProperty(
      "--hex-3",
      "var(--code-input_special-chars_" + hexCode[3] + ")",
    );

    // Handle zero-width chars
    if (charWidth == 0)
      result.classList.add("code-input_special-char_zero-width");
  ...
  getCharacterColors(asciiCode) {
    ...
    if (!(asciiCode in this.cachedColors)) {
      ...
      backgroundColor = ("000000" + backgroundColor).substring(
        backgroundColor.length,
      ); // So valid HEX color with 6 characters
  ...
  getCharacterWidthEm(codeInput, char) {
    ...
    if (
      char != "\u0096" &&
      new RegExp("[\u{0000}-\u{001F}]|[\u{007F}-\u{009F}]", "g").test(char)
    ) {
    ...
    let font =
      getComputedStyle(codeInput.textareaElement).fontFamily +
      " " +
      getComputedStyle(codeInput.textareaElement).fontStretch +
      " " +
      getComputedStyle(codeInput.textareaElement).fontStyle +
      " " +
      getComputedStyle(codeInput.textareaElement).fontVariant +
      " " +
      getComputedStyle(codeInput.textareaElement).fontWeight +
      " " +
      getComputedStyle(codeInput.textareaElement).lineHeight; // Font without size

    // Lazy-load width of each character
    if (this.cachedWidths[font] == undefined) {
      this.cachedWidths[font] = {};
    }
    if (this.cachedWidths[font][char] != undefined) {
      // Use cached width
      return this.cachedWidths[font][char];
    }

    // Ensure font the same - 20px font size is where this algorithm works
    this.canvasContext.font = getComputedStyle(
      codeInput.textareaElement,
    ).font.replace(
      getComputedStyle(codeInput.textareaElement).fontSize,
      "20px",
    );
    ...
    if (
      ...
      let fontSize = Number(
        getComputedStyle(codeInput.textareaElement).fontSize.substring(
          0,
          getComputedStyle(codeInput.textareaElement).fontSize.length - 2,
        ),
      ); // Remove 20, make px
