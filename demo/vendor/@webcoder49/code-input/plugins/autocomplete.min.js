...
codeInput.plugins.Autocomplete = class extends codeInput.Plugin {
  ...
  updatePopup(a, b) {
    ...
    ((e.style.top = d.top + "px"),
      ...
      b || this.updatePopupCallback(e, c, c.selectionEnd, c.selectionStart));
  }
  afterElementsAdded(a) {
    ...
    if (
      (c.classList.add("code-input_autocomplete_invisiblepre"),
      c.setAttribute("aria-hidden", !0),
      a.templateObject.preElementStyled)
    )
      (c.classList.add("code-input_autocomplete_testpos"), a.appendChild(c));
    else {
      let b = document.createElement("code");
      (b.classList.add("code-input_autocomplete_testpos"),
        c.appendChild(b),
        a.appendChild(c));
    }
  ...
  getCaretCoordinates(a, b, c, d) {
    ...
    if (d) {
      ...
      for (
        f.textContent = b.value.substring(0, c),
