...
codeInput.plugins.GoToLine = class extends codeInput.Plugin {
  ...
  instructions = {
    ...
    guidanceFormat:
      "Wrong format. Enter a line number (e.g. 1) or a line number then colon then column number (e.g. 1:3).",
    guidanceLineRange: (current, max) =>
      `Line number (currently ${current}) should be between 1 and ${max} inclusive.`,
    guidanceColumnRange: (line, current, max) =>
      `On line ${line}, column number (currently ${current}) should be between 1 and ${max} inclusive.`,
    guidanceValidLine: (line) => `Press Enter to go to line ${line}.`,
    guidanceValidColumn: (line, column) =>
      `Press Enter to go to line ${line}, column ${column}.`,
  };
  ...
  afterElementsAdded(codeInput) {
    ...
    if (this.useCtrlG) {
      textarea.addEventListener("keydown", (event) => {
        this.checkCtrlG(codeInput, event);
      });
    }
  }
  ...
  checkPrompt(dialog, event) {
    ...
    // Invalid format
    if (
      querySplitByColons.length > 2 ||
      !/^[0-9:]*$/.test(dialog.input.value)
    ) {
    ...
    if (dialog.input.value) {
      if (lineNo < 1 || lineNo > maxLineNo) {
        ...
        dialog.guidance.textContent = this.instructions.guidanceLineRange(
          lineNo,
          maxLineNo,
        );
        return dialog.input.classList.add("code-input_go-to-line_error");
        ...
        if (columnNo < 0 || columnNo > maxColumnNo) {
          dialog.guidance.textContent = this.instructions.guidanceColumnRange(
            lineNo,
            columnNo,
            maxColumnNo,
          );
          ...
          if (columnNo === 0) {
            ...
            dialog.guidance.textContent =
              this.instructions.guidanceValidLine(lineNo);
          } else {
            dialog.guidance.textContent = this.instructions.guidanceValidColumn(
              lineNo,
              columnNo,
            );
          }
  ...
  showPrompt(codeInput) {
    if (
      codeInput.pluginData.goToLine == undefined ||
      codeInput.pluginData.goToLine.dialog == undefined
    ) {
      const textarea = codeInput.textareaElement;
      ...
      input.addEventListener("keyup", (event) => {
        return this.checkPrompt(dialog, event);
      });
      ...
      codeInput.pluginData.goToLine.dialog.classList.remove(
        "code-input_go-to-line_hidden-dialog",
      );
  ...
  goTo(textarea, lineNo, columnNo = 0) {
    ...
    if (lineNo > 0 && lineNo <= lines.length) {
      if (textarea.computedStyleMap) {
        ...
        lineHeight =
          fontSize * textarea.computedStyleMap().get("line-height").value;
      } else {
