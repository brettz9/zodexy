...
codeInput.plugins.FindAndReplace = class extends codeInput.Plugin {
  ...
  afterElementsAdded(codeInput) {
    ...
    if (this.useCtrlF) {
      textarea.addEventListener("keydown", (event) => {
        this.checkCtrlF(codeInput, event);
      });
    }
    if (this.useCtrlH) {
      textarea.addEventListener("keydown", (event) => {
        this.checkCtrlH(codeInput, event);
      });
    }
  }
  ...
  afterHighlight(codeInput) {
    if (
      codeInput.pluginData.findAndReplace != undefined &&
      codeInput.pluginData.findAndReplace.dialog != undefined
    ) {
      if (
        !codeInput.pluginData.findAndReplace.dialog.classList.contains(
          "code-input_find-and-replace_hidden-dialog",
        )
      ) {
        ...
        if (
          codeInput.pluginData.findAndReplace.dialog.findMatchState
            .numMatches == 0
        ) {
          // No more matches after editing
          codeInput.pluginData.findAndReplace.dialog.findInput.classList.add(
            "code-input_find-and-replace_error",
          );
        }
      }
    }
  ...
  text2RegExp(string, caseSensitive, stringIsRegexp) {
    ...
    return new RegExp(
      stringIsRegexp ? string : string.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
      caseSensitive ? "g" : "gi",
  ...
  updateMatchDescription(dialog) {
    ...
    if (dialog.findInput.value.length == 0) {
      ...
      dialog.matchDescription.textContent = this.instructions.matchIndex(
        dialog.findMatchState.focusedMatchID + 1,
        dialog.findMatchState.numMatches,
      );
    }
  }
    ...
    setTimeout(() => {
      if (oldValue == dialog.findInput.value) {
        ...
        if (oldValue.length > 0) {
          try {
            dialog.findMatchState.updateMatches(
              this.text2RegExp(
                dialog.findInput.value,
                dialog.findCaseSensitiveCheckbox.checked,
                dialog.findRegExpCheckbox.checked,
              ),
            );
          } catch (err) {
            if (err instanceof SyntaxError) {
              // Syntax error due to malformed RegExp
              dialog.findInput.classList.add(
                "code-input_find-and-replace_error",
              );
              // Only show last part of error message
              let messageParts = err.message.split(": ");
              dialog.matchDescription.textContent = this.instructions.error(
                messageParts[messageParts.length - 1],
              ); // Show only last part of error.
              return;
            } else {
              throw err;
            }
          ...
          if (dialog.findMatchState.numMatches > 0) {
            dialog.findInput.classList.remove(
              "code-input_find-and-replace_error",
            );
          } else {
            // No matches - error
  ...
  cancelPrompt(dialog, codeInput, event) {
    ...
    if (dialog.findMatchState.numMatches > 0) {
      ...
      codeInput.textareaElement.selectionStart =
        dialog.findMatchState.matchStartIndexes[
          dialog.findMatchState.focusedMatchID
        ];
      codeInput.textareaElement.selectionEnd =
        dialog.findMatchState.matchEndIndexes[
          dialog.findMatchState.focusedMatchID
        ];
    } else {
  ...
  showPrompt(codeInputElement, replacePartExpanded) {
    let dialog;
    if (
      codeInputElement.pluginData.findAndReplace == undefined ||
      codeInputElement.pluginData.findAndReplace.dialog == undefined
    ) {
      ...
      findCaseSensitiveCheckbox.classList.add(
        "code-input_find-and-replace_case-sensitive-checkbox",
      );
      findRegExpCheckbox.setAttribute("type", "checkbox");
      findRegExpCheckbox.title = this.instructions.findRegExp;
      findRegExpCheckbox.classList.add(
        "code-input_find-and-replace_reg-exp-checkbox",
      );

      matchDescription.textContent = this.instructions.start;
      matchDescription.classList.add(
      ...
      replaceDropdown.addEventListener("toggle", () => {
        ...
        replaceButton.classList.toggle(
          "code-input_find-and-replace_button-hidden",
        );
        replaceAllButton.classList.toggle(
          "code-input_find-and-replace_button-hidden",
        );
      });

      // To store the state of find-and-replace functionality
      dialog.findMatchState =
        new codeInput.plugins.FindAndReplace.FindMatchState(codeInputElement);
      ...
      replaceInput.addEventListener("input", (event) => {
        ...
        if (
          dialog.classList.contains("code-input_find-and-replace_hidden-dialog")
        ) {
          // Show prompt
      ...
      dialog.addEventListener("keyup", (event) => {
        ...
        if (event.key == "Escape")
          this.cancelPrompt(dialog, codeInputElement, event);
      });

      findInput.addEventListener("keyup", (event) => {
        this.checkFindPrompt(dialog, codeInputElement, event);
      });
      ...
      findCaseSensitiveCheckbox.addEventListener("click", (event) => {
        this.updateFindMatches(dialog);
      });
      findRegExpCheckbox.addEventListener("click", (event) => {
        this.updateFindMatches(dialog);
      });
      ...
      cancel.addEventListener("keypress", (event) => {
      ...
      if (dialog.selectionStart < dialog.selectionEnd) {
        ...
        let textToFind = codeInputElement.textareaElement.value.substring(
...
codeInput.plugins.FindAndReplace.FindMatchState = class {
  ...
  constructor(codeInputElement) {
    this.focusedMatchStartIndex =
      codeInputElement.textareaElement.selectionStart;
  ...
  clearMatches() {
    ...
    let tempSpans = this.codeInput.codeElement.querySelectorAll(
      ".code-input_find-and-replace_temporary-span",
    );
    for (let i = 0; i < tempSpans.length; i++) {
      // Replace with textContent as Text node
      tempSpans[i].parentElement.replaceChild(
        new Text(tempSpans[i].textContent),
        tempSpans[i],
      );
    }

    // Remove old matches
    let oldMatches = this.codeInput.codeElement.querySelectorAll(
      ".code-input_find-and-replace_find-match",
    );
    for (let i = 0; i < oldMatches.length; i++) {
      oldMatches[i].removeAttribute(
        "data-code-input_find-and-replace_match-id",
      ); // No match ID
      oldMatches[i].classList.remove("code-input_find-and-replace_find-match"); // No highlighting
      oldMatches[i].classList.remove(
        "code-input_find-and-replace_find-match-focused",
      ); // No focused highlighting
    }
  }
  ...
  updateMatches(searchRegexp) {
    ...
    let focusedMatchBlock = Math.floor(
      this.focusedMatchID / CODE_INPUT_FIND_AND_REPLACE_MATCH_BLOCK_SIZE,
    );
    ...
    while ((match = searchRegexp.exec(this.codeInput.value)) !== null) {
      ...
      let currentMatchBlock = Math.floor(
        matchID / CODE_INPUT_FIND_AND_REPLACE_MATCH_BLOCK_SIZE,
      );
  ...
  replaceOnce(replacementText) {
    if (
      this.numMatches > 0 &&
      replacementText !=
        this.codeInput.value.substring(
          0,
          this.matchStartIndexes[this.focusedMatchID],
          this.matchEndIndexes[this.focusedMatchID],
        )
    ) {
      // Go to next match
      ...
      this.codeInput.textareaElement.selectionStart =
        this.matchStartIndexes[this.focusedMatchID];
      this.codeInput.textareaElement.selectionEnd =
        this.matchEndIndexes[this.focusedMatchID];
  ...
  replaceAll(replacementText) {
    ...
    for (let i = 0; i < this.numMatches; i++) {
      ...
      this.codeInput.textareaElement.focus();
      this.codeInput.textareaElement.selectionStart =
        this.matchStartIndexes[i] + numCharsAdded;
      this.codeInput.textareaElement.selectionEnd =
        this.matchEndIndexes[i] + numCharsAdded;

      numCharsAdded +=
        replacementNumChars -
        (this.matchEndIndexes[i] - this.matchStartIndexes[i]);
  ...
  previousMatch() {
    this.focusMatch(
      (this.focusedMatchID + this.numMatches - 1) % this.numMatches,
    );
  }
  ...
  focusMatch(matchID = undefined) {
    if (matchID === undefined) {
      ...
      matchID = 0;
      while (
        matchID < this.matchStartIndexes.length &&
        this.matchStartIndexes[matchID] < this.focusedMatchStartIndex
      ) {
        matchID++;
      }
    ...
    // Delete old focus
    let oldFocused = this.codeInput.codeElement.querySelectorAll(
      ".code-input_find-and-replace_find-match-focused",
    );
    for (let i = 0; i < oldFocused.length; i++) {
      oldFocused[i].classList.remove(
        "code-input_find-and-replace_find-match-focused",
      );
    }

    // Highlight match block if needed
    let highlightedMatchBlock = Math.floor(
      matchID / CODE_INPUT_FIND_AND_REPLACE_MATCH_BLOCK_SIZE,
    );
    if (!this.matchBlocksHighlighted[highlightedMatchBlock]) {
      ...
      for (
        let i =
          CODE_INPUT_FIND_AND_REPLACE_MATCH_BLOCK_SIZE * highlightedMatchBlock;
        i <
        CODE_INPUT_FIND_AND_REPLACE_MATCH_BLOCK_SIZE *
          (highlightedMatchBlock + 1);
        i++
      ) {
        // Highlight match
        this.highlightMatch(
          i,
          this.codeInput.codeElement,
          this.matchStartIndexes[i],
          this.matchEndIndexes[i],
        );
      }
    }

    // Add new focus
    let newFocused = this.codeInput.codeElement.querySelectorAll(
      `.code-input_find-and-replace_find-match[data-code-input_find-and-replace_match-id="${matchID}"]`,
    );
    for (let i = 0; i < newFocused.length; i++) {
      newFocused[i].classList.add(
        "code-input_find-and-replace_find-match-focused",
      );
    }

    if (newFocused.length > 0) {
      this.codeInput.scrollTo(
        newFocused[0].offsetLeft - this.codeInput.offsetWidth / 2,
        newFocused[0].offsetTop - this.codeInput.offsetHeight / 2,
      ); // So focused match in centre of screen
    }
  }
  ...
  highlightMatch(matchID, currentElement, startIndex, endIndex) {
    const lines = currentElement.textContent
      .substring(startIndex, endIndex)
      .split("\n");
    let lineStartIndex = startIndex;
    for (let i = 0; i < lines.length; i++) {
      if (i == 0) {
        this.highlightMatchNewlineOnlyAtStart(
          matchID,
          currentElement,
          lineStartIndex,
  ...
  highlightMatchNewlineOnlyAtStart(
    matchID,
    currentElement,
    startIndex,
    endIndex,
    ...
    for (let i = 0; i < currentElement.childNodes.length; i++) {
      ...
      if (childElement.nodeType == 3) {
        // Text node
        if (
          i + 1 < currentElement.childNodes.length &&
          currentElement.childNodes[i + 1].nodeType == 3
        ) {
          // Can merge with next text node
          currentElement.childNodes[i + 1].textContent =
            childElement.textContent +
            currentElement.childNodes[i + 1].textContent; // Merge textContent with next node
        ...
        if (childText.length >= endIndex) {
          ...
          if (noInnerElements) {
            ...
            if (startSpan.textContent[0] == "\n") {
              ...
              startSpan.classList.add(
                "code-input_find-and-replace_start-newline",
              );
            }
