...
codeInput.plugins.Indent = class extends codeInput.Plugin {
  ...
  constructor(
    ...
    ((this.escTabToChangeFocus = !0),
      this.addTranslations(this.instructions, e));
  }
  ...
  afterElementsAdded(a) {
    ...
    (b.addEventListener("focus", () => {
      this.escTabToChangeFocus &&
        a.setKeyboardNavInstructions(this.instructions.tabForIndentation, !0);
    }),
    ...
    if (a.templateObject.preElementStyled)
      (c.appendChild(d),
        c.classList.add("code-input_autocomplete_test-indentation-width"),
        a.appendChild(c));
    else {
      let b = document.createElement("code");
      (b.appendChild(d),
        b.classList.add("code-input_autocomplete_test-indentation-width"),
        c.appendChild(b),
        a.appendChild(c));
    }
    ...
    (a.removeChild(c),
      (a.pluginData.indent = {
        automatedKeypresses: !1,
        indentationWidthPx: e,
      }));
  }
  checkTab(a, b) {
    var c = Math.max;
    if (
      !a.pluginData.indent.automatedKeypresses &&
      this.tabIndentationEnabled
    ) {
      if (this.escTabToChangeFocus) {
        if ("Escape" == b.key)
          return (
            (this.escJustPressed = !0),
            void a.setKeyboardNavInstructions(
        ...
        if ("Tab" != b.key)
          return "Shift" == b.key
            ? void 0
            : (a.setKeyboardNavInstructions(
                this.instructions.tabForIndentation,
                !1,
              ),
              void (this.escJustPressed = !1));
        if (!this.enableTabIndentation || this.escJustPressed)
          return (
            a.setKeyboardNavInstructions("", !1),
            void (this.escJustPressed = !1)
          );
      } else if ("Tab" != b.key) return;
      let d = a.textareaElement;
      if (
        (b.preventDefault(), !b.shiftKey && d.selectionStart == d.selectionEnd)
      )
        ((a.pluginData.indent.automatedKeypresses = !0),
          document.execCommand("insertText", !1, this.indentation),
          (a.pluginData.indent.automatedKeypresses = !1));
      else {
        let e = d.value.split("\n"),
          f = 0,
          g = d.selectionStart,
        ...
        for (let j = 0; j < e.length; j++)
          (((g <= f + e[j].length && h >= f + 1) ||
            (g == h && g <= f + e[j].length + 1 && h >= f)) &&
            (b.shiftKey
              ? e[j].substring(0, this.indentationNumChars) ==
                  this.indentation &&
                ((d.selectionStart = f),
              ...
              : ((d.selectionStart = f),
                ...
                (f += this.indentationNumChars))),
            (f += e[j].length + 1));
        ((d.selectionStart = g), (d.selectionEnd = h));
        const i = getComputedStyle(a).direction;
        "rtl" == i
          ? b.shiftKey
            ? a.scrollBy(a.pluginData.indent.indentationWidthPx, 0)
            : a.scrollBy(-a.pluginData.indent.indentationWidthPx, 0)
          : b.shiftKey
            ? a.scrollBy(-a.pluginData.indent.indentationWidthPx, 0)
            : a.scrollBy(a.pluginData.indent.indentationWidthPx, 0);
  ...
  checkEnter(a, b) {
    if (a.pluginData.indent.automatedKeypresses) return;
    ...
    for (
      let c = 0;
      c < j &&
      d[f].substring(c, c + this.indentationNumChars) == this.indentation;
      c += this.indentationNumChars
    )
      h++;
    let k = "";
    j != d[f].length &&
      ((k = d[f].substring(j)), (d[f] = d[f].substring(0, j)));
    let l = !1,
      m = "";
    ...
    ((a.pluginData.indent.automatedKeypresses = !0),
      ...
      (a.pluginData.indent.automatedKeypresses = !1),
      (c.selectionStart = n + h * this.indentationNumChars + 1),
    ...
    (f * p + 2 * p + o >= c.scrollTop + q &&
      a.scrollBy(0, +getComputedStyle(c).lineHeight.replace("px", "")),
      (a.value = c.value));
  }
  checkBackspace(a, b) {
    if (
      !a.pluginData.indent.automatedKeypresses &&
      "Backspace" == b.key &&
      1 != this.indentationNumChars
    ) {
      let c = a.textareaElement;
      c.selectionStart == c.selectionEnd &&
        a.value.substring(
          c.selectionStart - this.indentationNumChars,
          c.selectionStart,
        ) == this.indentation &&
        ((c.selectionStart -= this.indentationNumChars),
        b.preventDefault(),
  ...
  checkCloseBracket(a, b) {
    if (a.textareaElement.selectionStart == a.textareaElement.selectionEnd)
      for (let c in this.bracketPairs) {
        ...
        b.data == d &&
          a.value.substring(
            a.textareaElement.selectionStart - this.indentationNumChars,
