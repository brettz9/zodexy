...
codeInput.plugins.SelectTokenCallbacks = class extends codeInput.Plugin {
   ...
   */
  constructor(
    tokenSelectorCallbacks = codeInput.plugins.SelectTokenCallbacks.TokenSelectorCallbacks.createClassSynchronisation(),
    onlyCaretNotSelection = false,
    caretAtStartIsSelected = true,
    caretAtEndIsSelected = true,
    createSubTokens = false,
    partiallySelectedTokensAreSelected = true,
    parentTokensAreSelected = true,
  ) {
    ...
    this.partiallySelectedTokensAreSelected =
      partiallySelectedTokensAreSelected;
  ...
  afterElementsAdded(codeInputElement) {
    ...
    codeInputElement.pluginData.selectTokenCallbacks.lastSelectionStart =
      codeInputElement.textareaElement.selectionStart;
    codeInputElement.pluginData.selectTokenCallbacks.lastSelectionEnd =
      codeInputElement.textareaElement.selectionEnd;
    codeInputElement.pluginData.selectTokenCallbacks.selectedTokenState =
      new codeInput.plugins.SelectTokenCallbacks.SelectedTokenState(
        codeInputElement.codeElement,
        this.tokenSelectorCallbacks,
        this.onlyCaretNotSelection,
        this.caretAtStartIsSelected,
        this.caretAtEndIsSelected,
        this.createSubTokens,
        this.partiallySelectedTokensAreSelected,
  ...
  syncSelection(codeInputElement) {
    codeInputElement.pluginData.selectTokenCallbacks.selectedTokenState.updateSelection(
      codeInputElement.textareaElement.selectionStart,
      codeInputElement.textareaElement.selectionEnd,
    );
  }
...
codeInput.plugins.SelectTokenCallbacks.TokenSelectorCallbacks = class {
   ...
   */
  static createClassSynchronisation(
    selectedClass = "code-input_select-token-callbacks_selected",
  ) {
    return new codeInput.plugins.SelectTokenCallbacks.TokenSelectorCallbacks(
      ...
      (tokenContainer) => {
        ...
        let selectedClassTokens =
          tokenContainer.getElementsByClassName(selectedClass);
...
codeInput.plugins.SelectTokenCallbacks.SelectedTokenState = class {
  constructor(
    codeElement,
    tokenSelectorCallbacks,
    onlyCaretNotSelection,
    caretAtStartIsSelected,
    caretAtEndIsSelected,
    createSubTokens,
    partiallySelectedTokensAreSelected,
    parentTokensAreSelected,
  ) {
  ...
  updateSelection(selectionStart, selectionEnd) {
    this.selectChanged();
    if (!this.onlyCaretNotSelection || selectionStart == selectionEnd) {
      // Only deal with selected text if onlyCaretNotSelection is false.
  ...
  selectChanged() {
    if (this.createSubTokens) {
      ...
      let tempSpans = this.tokenContainer.getElementsByClassName(
        "code-input_select-token-callbacks_temporary-span",
      );
      while (tempSpans.length > 0) {
        // Replace with textContent as Text node
        // Use it like a queue, because as elements have their class name removed they are live-removed from the collection.
        tempSpans[0].parentElement.replaceChild(
          new Text(tempSpans[0].textContent),
          tempSpans[0],
        );
      }
    }
  ...
  updateSelectedTokens(currentElement, startIndex, endIndex) {
    ...
    if (
      this.parentTokensAreSelected &&
      currentElement !== this.tokenContainer
    ) {
      ...
      if (startIndex <= 0) {
        ...
        if (childText.length > endIndex) {
          ...
          if (this.partiallySelectedTokensAreSelected) {
            if (noInnerElements) {
              if (this.createSubTokens && startIndex != endIndex) {
                // Subtoken to create
        ...
        (this.caretAtEndIsSelected && childText.length >= startIndex) ||
        childText.length > startIndex
      ) {
