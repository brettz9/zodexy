...
codeInput.plugins.Indent = class extends codeInput.Plugin {
  ...
  instructions = {
    tabForIndentation:
      "Tab and Shift-Tab currently for indentation. Press Esc to enable keyboard navigation.",
    tabForNavigation:
      "Tab and Shift-Tab currently for keyboard navigation. Type to return to indentation.",
  };
  ...
  afterElementsAdded(codeInput) {
    ...
    textarea.addEventListener("focus", (event) => {
      if (this.escTabToChangeFocus)
        codeInput.setKeyboardNavInstructions(
          this.instructions.tabForIndentation,
          true,
        );
    });
    textarea.addEventListener("keydown", (event) => {
      this.checkTab(codeInput, event);
      this.checkEnter(codeInput, event);
      this.checkBackspace(codeInput, event);
    });
    textarea.addEventListener("beforeinput", (event) => {
      this.checkCloseBracket(codeInput, event);
    });
    ...
    if (codeInput.templateObject.preElementStyled) {
      ...
      testIndentationWidthPre.classList.add(
        "code-input_autocomplete_test-indentation-width",
      );
      ...
      testIndentationWidthCode.classList.add(
        "code-input_autocomplete_test-indentation-width",
      );
    ...
    codeInput.pluginData.indent = {
      automatedKeypresses: false,
      indentationWidthPx: indentationWidthPx,
    };
  }
  ...
  checkTab(codeInput, event) {
    ...
    if (this.escTabToChangeFocus) {
      ...
      if (event.key == "Escape") {
        ...
        codeInput.setKeyboardNavInstructions(
          this.instructions.tabForNavigation,
          false,
        );
        return;
    ...
    if (
      ...
      for (let i = 0; i < lines.length; i++) {
        if (
          (selectionStartI <= letterI + lines[i].length &&
            selectionEndI >= letterI + 1) ||
          (selectionStartI == selectionEndI &&
            selectionStartI <= letterI + lines[i].length + 1 &&
            selectionEndI >= letterI)
        ) {
          // + 1 so newlines counted
          // Starts before or at last char and ends after or at first char
          if (event.shiftKey) {
            if (
              lines[i].substring(0, this.indentationNumChars) ==
              this.indentation
            ) {
              // Remove first indent
              ...
              // Change selection
              if (selectionStartI > letterI) {
                // Indented outside selection
                selectionStartI = Math.max(
                  selectionStartI - this.indentationNumChars,
                  letterI,
                ); // Don't move to before indent
              }
      ...
      if (textDirection == "rtl") {
        if (event.shiftKey) {
          ...
          // Scroll left
          codeInput.scrollBy(
            -codeInput.pluginData.indent.indentationWidthPx,
            0,
          );
        }
      } else {
  ...
  checkEnter(codeInput, event) {
    ...
    let cursorPosInLine =
      lines[currentLineI].length - (letterI - inputElement.selectionEnd) + 1;
    for (let i = 0; i < cursorPosInLine; i += this.indentationNumChars) {
      if (
        lines[currentLineI].substring(i, i + this.indentationNumChars) ==
        this.indentation
      ) {
        numberIndents++;
      } else {
    ...
    if (this.bracketPairs != null) {
      for (let openingBracket in this.bracketPairs) {
        if (
          lines[currentLineI][lines[currentLineI].length - 1] == openingBracket
        ) {
          let closingBracket = this.bracketPairs[openingBracket];
          if (
            textAfterCursor.length > 0 &&
            textAfterCursor[0] == closingBracket
          ) {
    ...
    inputElement.selectionStart =
      selectionStartI + numberIndents * this.indentationNumChars + 1; // count the indent level and the newline character
    inputElement.selectionEnd = inputElement.selectionStart;

    // Scroll down to cursor if necessary
    let paddingTop = Number(
      getComputedStyle(inputElement).paddingTop.replace("px", ""),
    );
    let lineHeight = Number(
      getComputedStyle(inputElement).lineHeight.replace("px", ""),
    );
    let inputHeight = Number(
      getComputedStyle(codeInput).height.replace("px", ""),
    );
    if (
      currentLineI * lineHeight + lineHeight * 2 + paddingTop >=
      inputElement.scrollTop + inputHeight
    ) {
      // Cursor too far down
      codeInput.scrollBy(
        0,
        Number(getComputedStyle(inputElement).lineHeight.replace("px", "")),
      );
    }
